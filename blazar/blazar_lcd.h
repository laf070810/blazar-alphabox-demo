...
void GPIO_ConfigLCDDataPins(int);

//PD0~7,作为数据线
#define DATAOUT(x) GPIO_ClearPinsOutput(GPIOD, 0xff);GPIO_SetPinsOutput(GPIOD, x&0xff); //数据输出
#define DATAIN     GPIO_ReadLCDDataPins();   //数据输入
...
/*!
 * @brief Fill the region of row_start <= row <= row_end & column_start <= column <= column_end with a color
 *
 * @param row_start
 * @param row_end
 * @param column_start
